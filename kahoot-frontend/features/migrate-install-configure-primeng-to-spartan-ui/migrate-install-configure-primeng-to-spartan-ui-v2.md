...
#### `postcss.config.mjs`
Angular's `@angular/build:application` picks this up automatically and runs PostCSS
on every stylesheet entry in `angular.json`.
...
#### `src/styles.scss` — removed `@layer primeng` and `@import 'primeicons/primeicons.css'`
